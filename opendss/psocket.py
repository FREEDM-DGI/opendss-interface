...
def Main():
	#connect################################################################################################
	host = '192.168.56.101'	# hard corded host address
	#host = sys.argv[1]	# user inputed host address
	port = 5003	# if port is changed, then it should be changed in device server adapter too.
	# port = int(sys.argv[2])
	...
	while (1):	#loop is terminated by socket timer or otherwise use infinite loop
...
def updateOpenDSSEngine(str):
	...
	dssObj.Text.command = "compile 'C:\Program Files\OpenDSS\HILTestBed\HIL7NodesSystem.dss'"
	print('updated....')
	sys.exit(0)
